...
class ContentBasedRecommender {
  ...
  setOptions(options = {}) {
    ...
    // default values
    this.options.maxVectorSize = this.options.maxVectorSize || 100;
  }
  ...
  train(documents) {
    ...
    const docVectors = this._produceWordVectors(preprocessDocs, this.options.maxVectorSize);

    // step 3 - calculate similarities
    this.sortedSets = this._calculateSimilarities(docVectors);
  }
  ...
  export() {
    const data = {};
    Object.keys(this.sortedSets).forEach((id) => {
      data[id] = this.getSimilarDocuments(id);
    });
  ...
  import(object) {
    ...
    Object.keys(data).forEach((id) => {
      const similarDocuments = data[id];

      this.sortedSets[id] = new SortedSet();

      similarDocuments.forEach((similarDocument) => {
        this.sortedSets[id].add(similarDocument.id, 1 - similarDocument.score);
      });
    });
  }
  ...
  _produceWordVectors(processedDocuments, maxVectorSize) {
    // process tfidf
    ...
    for (let i = 0; i < processedDocuments.length; i += 1) {
      ...
      const maxSize = Math.min(maxVectorSize, items.length);
